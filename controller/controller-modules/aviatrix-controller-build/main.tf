...
resource "aws_network_interface" "eni-controller" {
  count           = var.num_controllers
  subnet_id       = (var.subnet == "new" ? "${aws_subnet.avtx_ctrl_subnet[count.index].id}" : var.subnet)
  security_groups = ["${aws_security_group.AviatrixSecurityGroup[count.index].id}"]
  tags            = {
...
resource "aws_instance" "aviatrixcontroller" {
  ...
  key_name                = (var.keypair == "" ? "${aws_key_pair.avtx_ctrl_key[count.index].key_name}" : var.keypair)
